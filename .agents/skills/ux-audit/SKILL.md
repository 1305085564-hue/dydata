...
Do **not** silently fall back to a fresh Playwright session for an authenticated app — the audit is worthless if you can't log in. If Chrome MCP isn't connected, stop and say: *"Open Chrome, click Connect in the Codex extension, then rerun."*
...
2. Check AGENTS.md, README, `package.json` `homepage` field
...
3. **Deep links** — URLs in AGENTS.md, docs, or a prior audit report
...
- Read AGENTS.md / README for "what this app does"
...
> *"Empty state on /app/clients shows no CTA. Reference (Codex.ai) shows 3 keyboard shortcuts + 'New chat' button in the same position."*
