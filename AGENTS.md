...
|------|------|
| `/login`、`/register` | 登录注册 |
| `/dashboard` | 员工日报填报、趋势图、排行榜、已下发反馈查看 |
| `/growth` | 成长分析 |
| `/analytics` | 数据分析 |
| `/violations` | 违规话术提交/管理（员工+管理端共用入口） |
| `/video-review` | 视频审核 |
| `/admin` | 管理后台首页（经营驾驶舱） |
| `/admin/content` | 批改台：异常内容复核、反馈下发、经验标记 |
| `/admin/analytics` | 经营分析 |
| `/admin/videos` | 素材库与视频资产管理 |
| `/admin/fulfillment` | 发布履约 |
| `/admin/modules`、`/admin/guidance`、`/admin/advice` 等 | 历史功能页，当前处于低维护或待收敛状态，改动前需确认是否仍在使用 |
...
## 废弃机制速查（防止 AI 记忆污染）
以下机制曾在设计文档或早期 migration 中出现，但**未接入真实业务或已被替代**，讨论产品逻辑时以「当前真实状态」为准，不要从旧文档里把它们当成在用功能：

| 旧机制 | 当前状态 | 说明 |
|--------|----------|------|
| `sop_review_scores` 六维度评分 | 已废弃 | 数据库表仍在（migration 050），但前后端零调用。批改台不使用打分，改为 `content_feedback_cards` 的「主要问题 + 改进反馈」 |
| SOP 5 卡点状态实时追踪 / 组长看板 / 组长日报 | 未上线 | 仅在 P0 设计文档与 migration 中出现；前端没有对应入口，`dashboard-workspace-header.test.tsx` 明确断言不展示「审核中心」「全域矩阵」 |
| 审核中心 / 全域矩阵 Tab | 未上线 | 属于早期 dashboard 设计，当前 dashboard 未实现 |
...
- **不要从旧设计文档里读出废弃功能**：P0 产品逻辑文档等文件仍保留历史设计，读完后必须对照 AGENTS.md「废弃机制速查」和真实代码（`src/app` 目录、`content-detail-dialog.tsx` 等）确认机制是否真正上线。
