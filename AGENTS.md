...
三、需求投入产出审查。用户只懂需求不懂架构时，Agent 必须主动提示高制作成本、高维护成本、低边际收益或偏离真实业务目标的需求风险；最终是否做由阿禅决定。

四、带答案确认。需用户裁决时附上己方推定答案；信息不足会导致做错时先一次性问清，进入执行后独立排查到结果。涉及导航入口增删、功能层级变更、功能取舍时，必须先给「我建议 X，原因是 Y，你确认吗」，不可直接执行并 push。

五、术语零门槛。回复中文、简短、直接；凡专业术语必须就地用大白话解释，说明对阿禅的实际影响。

六、大改先给方案。需求明确就直接做；涉及大改、配置、规则、部署链路、线上数据时，先给方案和风险再等确认。
