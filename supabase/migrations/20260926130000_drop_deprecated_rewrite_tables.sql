...
--   * to_regclass：下列四张废弃表在线上均存在
--   * 数据与休眠：rewrite_fixed_modes 2 行 / rewrite_length_presets 3 行 /
--     rewrite_workflows 1 行 / rewrite_workflow_steps 2 行；最后写入均为
--     2026-04-14 ~ 2026-04-15，n_tup_upd = 0、n_tup_del = 0（创建后再无写入）
...
--     处理方式：把同为废弃表（051 同批列入）且应用零引用的 rewrite_workflow_steps 一起删掉，
--     策略随表消失；**不去改写或删除任何存活表上的安全策略**。故 steps 必须排在工作流表之前。
--   * 应用侧：src/ scripts/ tests/ 对这四张表零引用
--
...
-- 已知未做（有意留给后续单独授权）：051 还列了 rewrite_variants / sop_review_scores 两张
--   废弃表，线上同样仍存在，但没有任何视图、外键或策略引用它们，不影响空库重放。
...
BEGIN
  ...
  FOREACH target IN ARRAY ARRAY['rewrite_workflow_steps', 'rewrite_workflows', 'rewrite_fixed_modes', 'rewrite_length_presets'] LOOP
...
-- ----------------------------------------------------------------------------
-- 1) 重建被删的表：从 044_rewrite_workspace.sql 取 create table 段
--    （rewrite_workflow_steps / rewrite_workflows / rewrite_length_presets），
--    从 046_rewrite_fixed_modes.sql 取 rewrite_fixed_modes 的 create table 段
--    与 `alter table rewrite_conversations add column selected_fixed_mode_id`。
-- 2) 回灌删除前的整份数据快照（2026-09-26 只读快照，共 10 行）：
