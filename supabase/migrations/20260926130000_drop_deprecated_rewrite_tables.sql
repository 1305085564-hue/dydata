...
--   本迁移用未被占用的版本号把这份清理补到线上（不新增其它前向变更）。
--   上线后已做「正名」收尾：占号的 051_cleanup_deprecated_tables.sql 删除（其内容即本文件），
--   并把被 CLI 静默跳过的 051b_leader_daily_reports.sql 改名为 051_leader_daily_reports.sql，
--   使「线上 051 = 组长日报表」在本地文件上如实可见（CLI 只接受 <数字版本>_name.sql）。
--
