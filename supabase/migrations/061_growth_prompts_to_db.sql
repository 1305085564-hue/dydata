-- ============================================================
-- 061: Growth prompts to ai_feature_config
-- ============================================================
...
- rewrite -> 改写建议
$prompt$
WHERE feature_key = 'growth_insight'
  AND NULLIF(btrim(COALESCE(system_prompt, '')), '') IS NULL;
...
- action -> 下一步动作
$prompt$
WHERE feature_key = 'growth_advice'
  AND NULLIF(btrim(COALESCE(system_prompt, '')), '') IS NULL;

-- rollback: UPDATE public.ai_feature_config SET system_prompt = NULL WHERE feature_key IN ('growth_insight', 'growth_advice');
