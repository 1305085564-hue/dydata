...
- **输入框纸层**：输入框本身即为纸上唯一的交互微触点，采用宣纸漫反射底（`bg-[#FAF8F4]/50`，聚焦时提亮 `focus:bg-white`）+ 暖砂发丝边 + 微投影（`shadow-2xs`），直接落座在纯白纸面上，外部不加任何有色装饰外壳。
...
- ❌ 页面最外层桌面大背景（L1）使用纯白或冷灰（必须使用 `#FBF9F5` 象牙暖底；但注意：L2 业务纸面与弹窗内部必须坚守 `#FFFFFF` 纯白以保通透）
