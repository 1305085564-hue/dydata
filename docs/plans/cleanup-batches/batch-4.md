...
- [ ] A-10: 旧版 AI 改写配置面板整文件死（`src/app/(app)/admin/ai-config/components/rewrite-client.tsx`，501 行，默认导出 `RewriteClient` 在 :199）
...
- [ ] A-11: dashboard 豁免死集群（3 文件 ≈655 行）
...
- [ ] A-13: topic-helpers 死导出清剿（`src/app/(app)/topics/topic-helpers.ts`，195 行）
