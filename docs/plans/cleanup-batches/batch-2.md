...
- [ ] A-15a: `updateExemption`（`src/app/(app)/admin/actions.ts:258`，止于 :343）
...
- [ ] A-15b: `clearExemption`（`admin/actions.ts:344`，止于 :391）
...
- [ ] A-15c: `adminUpdateReport`（`admin/actions.ts:519`，止于 :553）
  ...
  - 验证命令: 同引用证明。
- [ ] A-15d: `adminDeleteReport`（`admin/actions.ts:554`，止于下一个保留定义前，约 :830 前——中间函数以 Read 为准）
  - 引用证明: `rg -n "\badminDeleteReport\b" src scripts tests -g '!src/app/(app)/admin/actions.ts'` → 空。
  - 删除范围: L554 起至下一个 export 前。
  - 测试耦合: 无。
  - 验证命令: 同引用证明。
- [ ] A-15e: `removeMemberFromTeam`（`admin/actions.ts:831`）
...
- [ ] A-15f: `hasPendingExemptionRequest`（`src/app/(app)/dashboard/actions.ts:188`）
  ...
  - 验证命令: 同引用证明。
- [ ] A-15g: `reviewExemptionRequest`（`admin/actions.ts:463`，止于 :518）—— **条件项**
...
- [ ] A-15h: `submitExemptionRequest`（admin 重复版，`admin/actions.ts:392`，止于 :462）—— **待 NU-2 签字**
...
- [ ] A-16: 死 handler `buildSummaryResponse`（`src/app/api/admin/collaboration/handlers.ts:25`）、`buildOperatorsResponse`（`handlers.ts:47`）
...
- [ ] A-17: `/api/admin/collaboration/staff` 整路由死
...
- [ ] A-18: `/api/admin/collaboration/talents` 整路由死（同 A-17 模式）
