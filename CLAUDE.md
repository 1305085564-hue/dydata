...
| /admin/analytics | 经营分析（趋势图重构 + 人员折叠 + 爆款/时间/AI洞察 + 导粉趋势） | admin/owner |
| /analytics | 数据分析（所有登录用户可访问，member只看自己数据） | 登录 |
...
- migration 001-033 全部已执行（线上）
- migration 034（排行榜新列函数）已执行
- git config user.email = 1305085564@qq.com（Vercel Hobby 要求）
- git remote 已切 SSH：git@github.com:1305085564-hue/dydata.git
- ⚠️ 待做：git push + Vercel 部署上线

### 阶段三完成功能清单（2026-03-22）
P1 高频痛点：
- 豁免弹窗默认单日豁免 + 多日改为3/4/5天按钮选择
- 排行榜补齐10列（播放/涨粉/导粉/点赞/评论/分享/收藏/均播/2s跳出/5s完播）
- 管理后台所有列表分页（10人/页）+ 提交状态默认折叠已提交
- 员工端申请豁免入口 + 管理员审批流
- 踢人功能（owner专属）+ member_change_log记录

P2 功能接通：
- 数据分析页对所有登录用户开放（member只看自己）
- 管理后台新增导粉趋势图（仅admin/owner可见）
- AI洞察接通（/api/growth/ai-insight，单视频昨日复盘，有缓存）
- PK差距条动效重做（8维对比，进入视口展开动效）
- 诊断建议逻辑重做（对比团队均值，固定模板建议）
- 旧版日报提交入口删除
- 话题标签重做（仅干货/复盘两个大按钮，必选验证）

P3 收口：
- 全站Toast统一（feedbackToast）+ ConfirmDialog规范
- 空白态/加载态/异常态全覆盖
- 移动端适配收口（375px无溢出，44px触控区）
