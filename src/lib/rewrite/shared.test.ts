...
test("bootstrap 默认模型会跳过首个无可用路由的 model view", async () => {
  const db = buildBaseDb();
  db.rewrite_model_routes = db.rewrite_model_routes.filter((row) => row.model_view_id !== "model-default");
  const service = createFakeService(db);

  const payload = await rewrite.getRewriteBootstrapPayload(service as never);

  assert.equal(payload.defaults.modelViewId, "model-alt");
  assert.equal(payload.modelViews[0]?.id, "model-alt");
});

test("显式选择首个无可用路由的模型时会被拒绝", async () => {
  const db = buildBaseDb();
  db.rewrite_model_routes = db.rewrite_model_routes.filter((row) => row.model_view_id !== "model-default");
  const service = createFakeService(db);
  ...
  await assert.rejects(
    () =>
      rewrite.resolveRewriteSelections(service as never, {
        modelViewId: "model-default",
      }),
    /当前真实模型未配置可用执行渠道/,
  );
...
test("resolveRewriteSelections 默认模型不再依赖 is_default，而是取首个健康路由模型", async () => {
  const db = buildBaseDb();
  db.rewrite_model_views = [
    {
      id: "model-opus",
      key: "opus",
      label: "Opus",
  ...
  db.rewrite_model_routes = [
    {
      id: "route-flash-lite",
      model_view_id: "model-flash-lite",
      workflow_step_id: null,
      channel_id: "channel-1",
  ...
  const result = await rewrite.resolveRewriteSelections(service as never, {});

  assert.equal(result.selections.modelView.id, "model-flash-lite");
});
