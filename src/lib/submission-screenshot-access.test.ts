...
test("日报截图链接使用站内受保护读取接口", () => {
  assert.equal(
    ...
    "/api/submission-screenshots/file?path=user-1%2Faccount-1%2Fscreenshot_1%2Ftest+image.png"
  );
});

test("只解析受保护接口中的合法对象路径，兼容相对站内地址", () => {
  assert.equal(
    parseSubmissionScreenshotPath(
  ...
  assert.equal(
    parseSubmissionScreenshotPath(
      "/api/submission-screenshots/file?path=user-1%2Faccount-1%2Fscreenshot_1%2Ftest.png"
    ),
    "user-1/account-1/screenshot_1/test.png"
  );
  assert.equal(
...
test("视频提交只接受当前登录用户自己的日报截图", () => {
  ...
  const ownedRelative = "/api/submission-screenshots/file?path=user-1%2Faccount-1%2Fscreenshot_1%2Fa.png";
  ...
  assert.deepEqual(getOwnedSubmissionScreenshotPaths("user-1", [ownedRelative], "https://dydata.cc"), [
    "user-1/account-1/screenshot_1/a.png",
  ]);
