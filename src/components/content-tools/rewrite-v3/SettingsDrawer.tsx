...
export function SettingsDrawer({
  ...
  return (
    <div
      className={cn(
        "absolute inset-0 bg-white z-40 transition-all duration-150 flex flex-col",
        ...
        {activeTab === "skills" ? (
          editingSkill ? (
            ...
            <div className="space-y-3 animate-in fade-in duration-200">
              <div className="text-[12px] font-medium uppercase tracking-wider text-[#78716C]">
                {editingSkill.id ? "编辑个人技能" : "新建个人技能"}
              </div>

              <label className="block space-y-1">
                <span className="text-[12px] font-medium text-[#292524] pl-0.5">
                  技能名称 (必填)
                </span>
                <input
                  type="text"
                    ...
                    {bootstrap.modelViews.map((item) => {
                      ...
                      return (
                        <option
                          ...
                          value={item.id}
                          disabled={disabled}
                          className={disabled ? "text-[#78716C]" : ""}
                        >
                          {item.label}
                          {disabled ? " (已停用)" : ""}
                        </option>
                  ...
                  {privateSkills.map((skill) => (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        {skill.description && (
                          <p className="text-[12px] text-[#78716C] line-clamp-1 mt-0.5 leading-relaxed">
                            {skill.description}
                          </p>
                        )}
                      </div>
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            ...
            <div className="border border-[#E5E0D6] bg-white p-4 rounded-lg space-y-3 shadow-sm">
              ...
              <p className="text-[12px] text-[#78716C] leading-relaxed leading-normal mt-1">
