  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 px-4 py-3 flex items-center justify-between border-b border-stone-200/40">
        <span className="text-[12px] font-bold uppercase tracking-[0.2em] text-stone-400 font-mono">
          历史记录
        </span>
        <span className="text-[12px] text-stone-400 font-mono font-bold tabular-nums">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-4 scrollbar-none">
        {conversations.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center px-4 py-12">
            <FileText className="h-6 w-6 text-stone-300 animate-pulse" />
            <p className="text-[12px] font-bold text-stone-400 mt-2">暂无记录</p>
          </div>
        ) : (
          sections.map((section) => (
            <div key={section.key} className="space-y-1">
              <div className="text-[9px] font-bold text-stone-400 px-2 uppercase tracking-[0.16em]">
                {section.label}
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isActive
                          ? 'bg-[#8AA8C7]/15 text-stone-900 font-bold'
                    ...
                    >
                      ...
                      <ChevronRight className="h-3 w-3 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 text-stone-400" />
                    </button>
                  );
                })}
              </div>
