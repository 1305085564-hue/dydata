...
export function RewriteHistoryV3({
  ...
  return (
    <div
      className={cn(
        'shrink-0 h-full flex flex-col bg-zinc-150/40 border-r border-zinc-200/50 transition-all duration-300 ease-[cubic-bezier(0.25,1,0.5,1)]',
    ...
    >
      ...
      <div className="shrink-0 px-4 py-3 flex items-center justify-between border-b border-zinc-200/40">
        ...
        <span className="text-[10px] text-zinc-400 font-mono font-bold tabular-nums">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-4 scrollbar-none">
        {conversations.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center px-4 py-12">
            <FileText className="h-6 w-6 text-zinc-300 animate-pulse" />
            <p className="text-[11px] font-bold text-zinc-400 mt-2">暂无记录</p>
          </div>
        ) : (
          sections.map((section) => (
            <div key={section.key} className="space-y-1">
              <div className="text-[9px] font-bold text-zinc-400 px-2 uppercase tracking-[0.16em]">
                {section.label}
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isActive
                          ? 'bg-[#8AA8C7]/15 text-zinc-900 font-bold'
                          : 'text-zinc-550 hover:bg-zinc-200/50 hover:text-zinc-800'
                      )}
                    >
                      <div className="flex items-center gap-1.5 min-w-0 flex-1">
                        <span
                          className={cn(
                            ...
                            isActive ? 'bg-[#8AA8C7]' : 'bg-zinc-300 group-hover:bg-zinc-400'
                          )}
                        />
                      ...
                      <ChevronRight className="h-3 w-3 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 text-zinc-400" />
                    </button>
                  );
                })}
              </div>
