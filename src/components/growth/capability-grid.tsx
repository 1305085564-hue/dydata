...
const toneMap = {
  success: "border-zinc-200 bg-zinc-50 text-[#6FAA7D]",
  warning: "border-zinc-200 bg-zinc-50 text-[#D99E55]",
  danger: "border-zinc-200 bg-zinc-50 text-[#C9604D]",
          ...
          <div className="h-full rounded-xl border border-zinc-200 bg-white p-5 transition-[background-color,color,box-shadow,transform] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-[1px] hover:shadow-sm active:translate-y-0">
            <div className="flex h-full flex-col gap-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-[13px] font-semibold tracking-tight text-zinc-800">{item.name}</div>
                  <div className="mt-1 text-[11px] text-zinc-500">{item.metricLabel}</div>
                </div>
              ...
              <div className="text-[24px] font-semibold font-mono tabular-nums tracking-tight text-zinc-800">
                <MetricValue value={item.metricValue} text={item.metricText} />
              </div>

              <div className="mt-auto space-y-2 rounded-xl border border-zinc-200 bg-white p-3">
                <div className="flex items-center gap-2 text-[11px] text-zinc-500">
                  <span className={cn("h-2 w-2 rounded-full", signalMap[item.sample.signal])} />
                  <span>{item.sample.label}</span>
                </div>
                <p className="text-[13px] leading-[1.7] text-zinc-500">{item.sample.hint}</p>
              </div>
            </div>
          </div>
        </motion.div>
