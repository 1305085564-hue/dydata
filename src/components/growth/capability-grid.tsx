    ...
    return (
      <MotionCard className="border-[var(--color-border)] bg-[var(--color-surface)]">
        <div className="space-y-3 p-5">
          ...
          <p className="text-sm text-[var(--color-text-secondary)]">数据不足，先连续提交数据后再看能力分布。</p>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {Array.from({ length: 6 }).map((_, index) => (
              <div key={index} className="rounded-[10px] border border-dashed border-[var(--color-border)] px-3 py-2 text-sm text-[var(--color-text-secondary)]">
                --
              </div>
            ))}
          </div>
          ...
          <MotionCard index={index} className="h-full border-[var(--color-border)] bg-[var(--color-surface)]">
            <div className="flex h-full flex-col gap-4 p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold tracking-[-0.02em] text-[var(--color-text-primary)]">{item.name}</div>
                  <div className="mt-1 text-xs text-[var(--color-text-secondary)]">{item.metricLabel}</div>
                </div>
              ...
              <div className="mt-auto space-y-2 rounded-[12px] border border-[var(--color-border)] bg-[rgba(255,255,255,0.7)] p-3">
                <div className="flex items-center gap-2 text-xs text-[var(--color-text-secondary)]">
                  <span className={cn("h-2.5 w-2.5 rounded-full", signalMap[item.sample.signal])} />
                  <span>{item.sample.label}</span>
                </div>
                <p className="text-sm leading-6 text-[var(--color-text-secondary)]">{item.sample.hint}</p>
              </div>
            </div>
          </MotionCard>
        </motion.div>
