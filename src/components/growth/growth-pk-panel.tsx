  ...
  return (
    <div ref={ref} className="flex h-2 w-full overflow-hidden rounded-full bg-stone-100">
      <motion.div
        ...
        style={{ backgroundColor: leftLeads ? "#1C1917" : "#78716C" }}
        initial={{ width: 0 }}
      ...
      <div className="flex flex-1 justify-end">
        <motion.div
          ...
          style={{ backgroundColor: leftLeads ? "#78716C" : "#1C1917" }}
          initial={{ width: 0 }}
...
function WinBadge({ leftLeads, tied }: { leftLeads: boolean; tied: boolean }) {
  ...
  return leftLeads ? (
    <span className="rounded-full bg-stone-50 px-2 py-0.5 text-[12px] text-stone-900">领先</span>
  ) : (
    <span className="rounded-full bg-stone-50 px-2 py-0.5 text-[12px] text-stone-500">落后</span>
  );
}
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1.5">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[12px] uppercase tracking-[0.25em] text-stone-500">PK Compare</span>
              <div className="h-px flex-1 bg-stone-200" />
            </div>
            <h2 className="text-[24px] font-medium tracking-tight text-stone-900">PK 对比</h2>
          ...
          {total > 0 && (
            <div className="shrink-0 rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-center">
              <div className="text-[18px] font-medium tabular-nums text-stone-900">
                {winCount}/{total}
              </div>
              <div className="text-[12px] uppercase tracking-[0.25em] text-stone-500">领先</div>
            </div>
          )}
        </div>
        {!rows.length ? (
