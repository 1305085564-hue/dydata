  ...
  if (!capabilityCards.length) {
    return (
      <div className="rounded-2xl border border-stone-200 bg-white p-5">
        <h2 className="text-[24px] font-medium tracking-tight text-stone-900">六维能力</h2>
        <p className="mt-2 text-[13px] leading-[1.7] text-stone-500">数据不足，先连续提交数据后再看能力分布。</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <select
          ...
          className="rounded-xl border border-stone-200 bg-white px-2.5 py-1 text-[12px] text-stone-900 outline-none hover:border-stone-300 focus:border-stone-900"
        >
      ...
      <div className="flex items-center justify-center" style={{ maxHeight: 240 }}>
        ...
        >
          ...
          {compareScores.map((score, i) => {
            ...
            return (
              <circle key={`cmp-${i}`} cx={x} cy={y} r={3} fill="#A8A29E" stroke="white" strokeWidth="1.5">
      ...
      <div className="flex items-center justify-center gap-3">
        <div className="inline-flex items-center gap-1.5 rounded-full border border-stone-200 bg-white px-3 py-1.5 text-[12px]">
          <span className="h-2 w-2 rounded-full bg-[#6FAA7D] ring-1 ring-white" />
          <span className="text-stone-500">最强</span>
          <span className="font-medium text-stone-900 tabular-nums">{capabilityCards[strongIndex]?.name}</span>
        </div>
        <div className="inline-flex items-center gap-1.5 rounded-full border border-stone-200 bg-white px-3 py-1.5 text-[12px]">
          <span className="h-2 w-2 rounded-full bg-[#C9604D] ring-1 ring-white" />
          <span className="text-stone-500">最弱</span>
          <span className="font-medium text-stone-900 tabular-nums">{capabilityCards[weakIndex]?.name}</span>
        </div>
      </div>
    </div>
  );
