    ...
    <div className={className}>
      ...
      >
        {diagnosisItems.map((item) => {
          ...
          return (
            <motion.article
              ...
              className="group flex flex-col rounded-xl border border-stone-200 bg-white p-4 transition-[background-color,color,box-shadow,transform] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-[1px] hover:shadow-sm active:translate-y-0"
            >
              {/* 头部 */}
              <div className="flex items-start justify-between gap-2">
                ...
                <span
                  className={`shrink-0 rounded-full px-2 py-0.5 text-[12px] font-medium ${
                    isWeak
                      ? "bg-stone-50 text-[#D99E55]"
                      : "bg-stone-50 text-[#6FAA7D]"
                  }`}
                >
                  {isWeak ? "优先处理" : "继续放大"}
                </span>
        ...
        {diagnosisItems.length === 0 ? (
          <div className="col-span-2 rounded-xl border border-dashed border-stone-200 bg-stone-50 p-4 text-[13px] text-stone-500">
            团队数据不足，暂时还不能给出有效对标。
          </div>
        ) : null}
      </motion.div>
