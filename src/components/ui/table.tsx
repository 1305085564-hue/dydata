...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    ...
    >
      <table
        data-slot="table"
        className={cn("w-full caption-bottom text-[13px] text-zinc-800 tabular-nums", className)}
        {...props}
      />
    </div>
  )
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-9 px-3 text-left align-middle whitespace-nowrap text-[10px] uppercase tracking-[0.25em] font-medium text-zinc-400 [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
