    ...
    >
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 bg-zinc-50 rounded-lg p-2.5 mb-4 border border-zinc-100 text-xs">
          <div
            title={
              item.summary.averagePlayCount === null
                ? "暂无播放量≥10,000 的合格成片数据"
                : undefined
            }
          >
            ...
            <div
              className={`font-semibold tabular-nums text-sm mt-0.5 ${item.summary.averagePlayCount === null ? "text-zinc-400 text-xs font-normal" : "text-zinc-800"}`}
            >
          ...
          <div
            title={
              item.summary.bestPlayCount === null
                ? "暂无播放量≥10,000 的合格成片数据"
                : undefined
            }
          >
            ...
            <div
              className={`font-semibold tabular-nums text-sm mt-0.5 ${item.summary.bestPlayCount === null ? "text-zinc-400 text-xs font-normal" : "text-zinc-900"}`}
            >
      ...
      <div className="flex items-center justify-between gap-2 pt-2 border-t border-zinc-100 min-w-0">
        <span className="text-xs text-zinc-500 truncate min-w-0 font-normal">
          {item.summary.qualifiedWorkCount} 条成片 ·{" "}
          {formatDate(item.latestWorkedAt)}
        </span>
        {isScripting ? (
          <span className="px-2.5 py-1 rounded-md bg-zinc-100 text-zinc-700 border border-zinc-200/80 text-xs font-medium shrink-0">
            脚本中
          </span>
        ) : (
          <button
...
export function TodayFocusSection({
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3].map((item) => (
            <div
              key={item}
              className="h-56 bg-zinc-100 rounded-xl border border-zinc-200 p-5 animate-pulse"
            />
          ))}
        </div>
      </section>
  ...
  return (
    <section className="mb-6">
      ...
      {error ? (
        <div className="rounded-xl border border-zinc-200 bg-zinc-100/60 p-8 text-center">
          <AlertCircle className="w-6 h-6 text-[#DC2626] mx-auto mb-2" />
          <p className="text-sm font-medium text-[#DC2626]">今日聚焦加载失败</p>
