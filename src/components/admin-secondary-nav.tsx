...
// Note: 转化中心、合规审核 已迁至 /violations 避坑案例下 perspective tabs。
...
function getCardClassName(active: boolean) {
  return cn(
    "admin-subnav-link group flex min-w-[220px] flex-1 shrink-0 flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-5 text-left transition-[background-color,border-color] duration-150 ease-out hover:border-[#D97757]/40 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-zinc-300 sm:min-w-[240px] xl:min-w-[200px]",
        ...
        const content = (
          <>
            <div className="admin-subnav-link-head flex items-start gap-3">
              <span className="admin-subnav-link-icon flex size-11 items-center justify-center rounded-2xl border border-zinc-200 bg-zinc-50 text-zinc-700 transition group-hover:border-[#D97757]/30 group-hover:text-[#D97757]">
                <Icon className="size-5" />
              </span>
              <span className="admin-subnav-link-title flex-1 pt-1 text-[18px] font-semibold tracking-[-0.02em] text-zinc-900">
                {item.label}
              </span>
            </div>
            <span className="admin-subnav-link-description app-shell-metric-hint text-[13px] font-normal leading-6 text-zinc-500">
              {item.description}
            </span>
          </>
        );
