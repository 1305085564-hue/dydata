    ...
    <button type="button" className="block w-full text-left" onClick={handleClick}>
      <div
        className={cn(
          ...
          isUnread ? "border-zinc-200 bg-white" : "border-zinc-100 bg-zinc-50/60",
        )}
      >
        <span
          className={cn(
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] text-zinc-500">{relativeTime(row.created_at)}</span>
          </div>
          <div
            className={cn(
      ...
      <SheetContent side="right" className="w-full max-w-md sm:w-[420px]">
        ...
        <SheetBody className="space-y-2">
          {loading && list.length === 0 ? (
            <div className="flex h-full min-h-[200px] flex-col items-center justify-center gap-2 text-center text-zinc-500">
              <Bell className="size-8 animate-pulse stroke-[1.25]" />
              <div className="text-[13px]">正在读取通知</div>
              <div className="text-[12px] text-zinc-500">首次打开时会拉取待办和动态</div>
            </div>
          ) : list.length === 0 ? (
            <div className="flex h-full min-h-[200px] flex-col items-center justify-center gap-2 text-center text-zinc-500">
              <Inbox className="size-8 stroke-[1.25]" />
              <div className="text-[13px]">暂无通知</div>
              <div className="text-[12px] text-zinc-500">有待办或动态会出现在这里</div>
            </div>
          ) : (
            list.map((row) => (
              <NotificationCard
