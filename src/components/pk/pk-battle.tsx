...
function WinBadge({ leftLeads, tied }: { leftLeads: boolean; tied: boolean }) {
  if (tied) return <span className="text-xs text-zinc-500">持平</span>;
  return leftLeads ? (
    <span className="rounded-full bg-[#8AA8C7]/10 px-2 py-0.5 text-xs font-medium text-[#8AA8C7]">领先</span>
  ) : (
    <span className="rounded-full bg-[rgba(0,0,0,0.06)] px-2 py-0.5 text-xs font-medium text-zinc-500">落后</span>
  );
}
    ...
    <MotionCard className={className}>
      <div className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold tracking-[-0.02em] text-zinc-800">PK 对比</h2>
            ...
            >
              ...
              <div className="mt-2 flex justify-between gap-2 text-xs text-zinc-500">
                <span className="min-w-0 truncate">
                  <span className="font-medium text-zinc-800">{playerA.name}</span>{" "}
                ...
                <span className="min-w-0 shrink-0 text-right">
                  ...
                  <span className="font-medium text-zinc-800">{playerB.name}</span>
                </span>
              </div>
            </motion.div>
          ))}
