...
export function EditorialEpigraph({ quote, author, className = "" }: EditorialEpigraphProps) {
  return (
    <div className={`relative py-4 my-2 border-l-2 border-[#D97757]/60 pl-4 bg-gradient-to-r from-[#F1F1F0]/60 to-transparent rounded-r-lg ${className}`}>
      <p className="not-italic text-[13.5px] leading-[1.7] text-[#292524]/90 tracking-normal">
        “{quote}”
      </p>
      {author ? (
...
export function EditorialSidenote({ children, className = "" }: EditorialSidenoteProps) {
  return (
    <aside className={`text-[12.5px] leading-[1.65] text-[#78716C] border-t border-[#E2E2DF]/80 pt-2.5 mt-3 flex items-start gap-2 ${className}`}>
