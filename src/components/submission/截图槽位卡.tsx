...
function getStatusVisual(status: SubmissionSlotStatus, isWarning: boolean): StatusVisual {
  ...
  return { label: "待上传", dotClass: "bg-zinc-300", textClass: "text-zinc-700", borderClass: "border-zinc-300" };
}
      ...
      <header className="flex items-center justify-between gap-3 pb-3">
        <div className="flex min-w-0 items-center gap-2.5">
          <div className="flex min-w-0 flex-col gap-0.5">
            <span className="text-[13px] font-medium leading-tight tracking-tight text-zinc-900">
              {title}
            </span>
            {description ? (
              <span className="truncate text-[12px] leading-tight text-zinc-500">
                {description}
              </span>
            ) : null}
          </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          {status !== "empty" && (
            <span
              className={cn(
                "inline-flex items-center gap-1.5 rounded-full border bg-white px-2 py-[3px] text-[12px] font-medium transition-colors duration-150",
                visual.borderClass,
                visual.textClass,
              )}
            >
      ...
      <div className="grid gap-4 sm:grid-cols-[minmax(0,260px)_minmax(0,1fr)]">
        ...
        >
          <motion.div 
            ...
            className={cn(
              "group/drop relative flex aspect-[4/3] w-full items-center justify-center overflow-hidden rounded-xl border bg-zinc-100/60 text-zinc-500 transition-[background-color,box-shadow,transform] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)]",
              !assetUrl && !isProcessing && !isDragOver && (isTouchDevice ? "border-solid border-zinc-300 bg-white active:scale-[0.98]" : "border-dashed hover:border-[#D97757]/45 hover:bg-[#FDF9F7]/60 hover:text-[#D97757]"),
              ...
              assetUrl && !isProcessing && "bg-white p-0 border-zinc-300",
                  ...
                  render={
                    <div className="group/img relative h-full w-full cursor-zoom-in">
                      ...
                      <div className="pointer-events-none absolute inset-0 flex items-end justify-end bg-gradient-to-t from-black/35 via-transparent to-transparent p-2 opacity-0 transition-opacity duration-150 group-hover/img:opacity-100">
                        <span className="inline-flex items-center gap-1 rounded-full bg-white/90 px-2 py-0.5 text-[12px] font-medium text-zinc-700 backdrop-blur">
        ...
        <div className="flex min-h-[150px] flex-col gap-2.5">
          {ocrSummary && ocrSummary.length > 0 ? (
            <div className="flex min-h-0 flex-1 flex-col rounded-xl bg-zinc-100/80 p-3">
              <div className="mb-1.5 flex items-center justify-between gap-2">
                <span className="inline-flex items-center gap-1.5 text-[12px] font-medium uppercase tracking-[0.22em] text-zinc-500">
                  <span className="size-1 rounded-full bg-[#D97757]" />
                  AI 识别结果
                </span>
                {fileName ? (
                  <span className="max-w-[140px] truncate text-[12px] tracking-wide text-zinc-500">
                    {fileName}
                  </span>
                ) : null}
              </div>
            ...
            <div className="flex flex-1 items-center justify-center rounded-xl bg-zinc-100/80 p-4">
              <div className="flex flex-col items-center gap-1.5">
                ...
                <span className="text-[12px] tracking-[0.18em] text-zinc-500">PARSING METRICS</span>
              </div>
            </div>
          ) : (
            <div className="flex flex-1 items-center justify-center rounded-xl bg-zinc-100/60 p-4">
              ...
              <span className="ml-3 text-[12px] tracking-wide text-zinc-500">等待识别结果</span>
            </div>
          )}
        </div>
      </div>
