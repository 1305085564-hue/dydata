...
import { INTERACTION_SCORE_FORMULA_TEXT } from "@/lib/metrics";
...
export function InteractionTrend({
  ...
  const continuousRows = useMemo(() => {
    ...
    return 补全连续日期(sorted.map((item) => item.date), today).map((date) => {
      ...
      return {
        date,
        personal: item?.score ?? null,
        team: item?.teamAverageScore ?? null,
      };
    });
  }, [sorted, today]);
      ...
      <div className="flex flex-col gap-2 border-b border-zinc-200 pb-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-1.5">
            <h3 className="text-[18px] font-semibold tracking-tight text-zinc-900">互动质量分趋势</h3>
