...
function CustomTooltip({
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white p-2.5 shadow-[0_4px_12px_-6px_rgba(15,23,42,0.06)]">
      <p className="text-[12px] font-medium tracking-tight text-zinc-700">{label}</p>
    ...
    <div className="space-y-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-0.5">
          <h3 className="text-[16px] font-medium tracking-tight text-zinc-900">导粉趋势</h3>
          <p className="text-[13px] text-zinc-600">按最近 {preset === "7d" ? "7" : "30"} 天查看团队导粉变化</p>
        </div>
        <div className="inline-flex w-fit rounded-lg border border-zinc-200 bg-zinc-50 p-0.5">
          {(["7d", "30d"] as Preset[]).map((p) => (
            <Button
              ...
              variant="ghost"
              className={cn(
                "h-7 rounded-md px-2 text-[12px] font-normal text-zinc-500 shadow-none transition-[background-color,color] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)]",
                preset === p && "bg-white text-zinc-900"
              )}
              onClick={() => setPreset(p)}
            >
              {p === "7d" ? "近7天" : "近30天"}
