      ...
      <AnimatePresence mode="wait">
                  ...
                  <div className="space-y-4 flex-1">
                    ...
                    <div className="space-y-3">
                      <div className="rounded-lg bg-zinc-50 border border-zinc-200 p-3">
                        <p className="text-[10px] uppercase tracking-[0.25em] font-medium text-zinc-400 mb-1">数据证据</p>
                        <p className="text-[13px] font-medium text-zinc-700 leading-[1.7]">{insight.best_direction.evidence}</p>
                      </div>

                      <div>
                        <p className="text-[10px] uppercase tracking-[0.25em] font-medium text-zinc-400 mb-1">建议行动</p>
                        <p className="text-[13px] leading-[1.7] text-zinc-600">{insight.best_direction.recommendation}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-4 flex-1">
                    ...
                    <div className="space-y-3">
                      <div className="rounded-lg bg-zinc-50 border border-zinc-200 p-3">
                        <p className="text-[10px] uppercase tracking-[0.25em] font-medium text-zinc-400 mb-1">数据证据</p>
                        <p className="text-[13px] font-medium text-zinc-700 leading-[1.7]">{insight.worst_direction.evidence}</p>
                      </div>

                      <div>
                        <p className="text-[10px] uppercase tracking-[0.25em] font-medium text-zinc-400 mb-1">建议行动</p>
                        <p className="text-[13px] leading-[1.7] text-zinc-600">{insight.worst_direction.recommendation}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {insight.validated_experiments && insight.validated_experiments.length > 0 && (
                <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="text-[13px] font-semibold text-zinc-800">已验证测试结论</h4>
                  </div>
                  <ul className="space-y-2">
                    {insight.validated_experiments.map((exp, idx) => (
                      <li key={idx} className="text-[13px] text-zinc-600 flex items-start gap-2 leading-[1.7]">
        ...
        {!insight && !error && !loading && (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="rounded-2xl border border-zinc-200 bg-white p-12 text-center shadow-sm"
          >
