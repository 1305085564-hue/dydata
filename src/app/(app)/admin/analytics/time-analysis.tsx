    ...
    <div className="space-y-4">
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.72fr)]">
        <div className="space-y-1">
          <h3 className="text-[18px] font-medium tracking-tight text-stone-900">时间维度分析</h3>
          <p className="max-w-2xl text-[13px] leading-[1.7] text-stone-500">
          ...
          <div className="relative space-y-1">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p
                  className={cn(
                    "mt-0.5 text-[13px] font-medium tracking-tight",
                    recommendationMeta.hasRecommendation ? "text-stone-900" : "text-stone-900",
                  )}
                >
            ...
            <p className="line-clamp-2 text-[12px] leading-[1.6] text-stone-500">{recommendationMeta.description}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(280px,0.55fr)]">
        <section className="self-start rounded-2xl border border-stone-200 bg-white p-4 shadow-[var(--shadow-card)] ">
          <div className="mb-4 grid gap-2 md:grid-cols-4">
            <div className="rounded-xl border border-stone-200 bg-white px-3 py-2">
              <p className="text-[12px] tracking-[0.12em] text-stone-500">有效样本数</p>
              <p className="mt-1 text-[18px] font-medium text-stone-700 tabular-nums tracking-tight">{heatmapData.totalEligible}</p>
              <p className="mt-0.5 truncate text-[12px] text-stone-500">已同时具备播放数据和可解析发布时间</p>
            </div>

            <div className="rounded-xl border border-stone-200 bg-white px-3 py-2">
              <p className="text-[12px] tracking-[0.12em] text-stone-500">未纳入统计数</p>
              <p className="mt-1 text-[18px] font-medium text-stone-700 tabular-nums tracking-tight">{excludedCount}</p>
            ...
            <div className="rounded-xl border border-stone-200 bg-white px-3 py-2">
              <p className="text-[12px] tracking-[0.12em] text-stone-500">推荐窗口置信度</p>
              <p className="mt-1 text-[18px] font-medium text-stone-700 tabular-nums tracking-tight">
            ...
            <div className="rounded-xl border border-stone-200 bg-white px-3 py-2">
              <p className="text-[12px] tracking-[0.12em] text-stone-500">最高播放中位数</p>
              <p className="mt-1 text-[18px] font-medium text-stone-700 tabular-nums tracking-tight">{formatPlayCount(heatmapData.maxMedianPlay || null)}</p>
              <p className="mt-0.5 truncate text-[12px] text-stone-500">用于定义热力图颜色强弱</p>
            </div>
          </div>
          ...
          <div className="mb-4 rounded-xl border border-stone-200 bg-stone-50 p-3">
            <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
              <div className="space-y-1">
                <p className="text-[13px] font-medium text-stone-900">推荐窗口说明</p>
                <p className="line-clamp-2 text-[12px] leading-[1.6] text-stone-500">{recommendationMeta.description}</p>
              </div>
              {recommendationMeta.hasRecommendation ? (
                <div className="rounded-lg border border-stone-200 border-l-[2px] border-l-[#D97757] bg-white px-3 py-2 text-[13px]">
                  <p className="font-medium text-stone-900">{recommendationMeta.title}</p>
                ...
                <div className="rounded-xl border border-stone-200 bg-white px-4 py-3 text-[13px] text-stone-500">
          ...
          <div className="overflow-x-auto pb-1">
            <div className="min-w-[760px]">
              ...
              >
                <div />
                {HOURS.map((hour) => (
                  <div key={hour} className="text-center text-[12px] font-normal text-stone-500">
              ...
              <div className="space-y-1">
                {WEEKDAYS.map((day, weekdayIndex) => (
                  <div
                    ...
                    style={{ gridTemplateColumns: "56px repeat(24, minmax(18px, 1fr))" }}
                  >
                    <div className="pr-1 text-[13px] font-medium text-stone-700">{day}</div>
                    {HOURS.map((hour) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "group relative aspect-square min-h-5 rounded-lg border transition duration-200 focus-visible:border-stone-500 focus-visible:outline-none focus-visible:shadow-sm focus-visible:ring-1 focus-visible:ring-stone-900/5",
          ...
          <div className="mt-3 flex flex-col gap-2 rounded-xl border border-stone-200 bg-stone-50 p-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-3 text-[13px] text-stone-700">
        ...
        <aside className="space-y-2.5 self-start rounded-2xl border border-stone-200 bg-white p-3 shadow-[var(--shadow-card)] ">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-[12px] font-normal tracking-[0.12em] text-stone-500">
              <MousePointer2 className="size-3.5" />
              时段详情
            </div>
            {selectedCell ? (
              <div className="inline-flex items-center gap-1 rounded-full border border-stone-200 bg-white px-2.5 py-1 text-[12px] text-stone-500">
          ...
          {activeCell && activeSummary ? (
            <>
              <div className="space-y-1">
                <div
                  className={cn(
                    "inline-flex items-center gap-2 rounded-full px-3 py-1 text-[12px] font-normal",
                    activeIsRecommended
              ...
              <div className="space-y-2.5">
                <div className="rounded-xl border border-stone-200 bg-white p-3">
                  <p className="text-[12px] tracking-[0.12em] text-stone-500">当前时段</p>
                  <p className="mt-1 text-[18px] font-medium text-stone-700 tabular-nums tracking-tight">
                ...
                <div className="grid gap-2 sm:grid-cols-3 xl:grid-cols-1">
                  <div className="rounded-xl border border-stone-200 bg-white p-3">
                    <p className="text-[12px] tracking-[0.12em] text-stone-500">样本数</p>
                    <p className="mt-0.5 text-[18px] font-medium text-stone-700 tabular-nums tracking-tight">{activeSummary.count} 条</p>
                  </div>
                  <div className="rounded-xl border border-stone-200 bg-white p-3">
                    <p className="text-[12px] tracking-[0.12em] text-stone-500">播放中位数</p>
                    <p className="mt-0.5 text-[18px] font-medium text-stone-700 tabular-nums tracking-tight">{formatPlayCount(activeSummary.medianPlay)}</p>
                  </div>
                  <div className="rounded-xl border border-stone-200 bg-white p-3">
                    <p className="text-[12px] tracking-[0.12em] text-stone-500">总播放</p>
                    <p className="mt-0.5 text-[18px] font-medium text-stone-700 tabular-nums tracking-tight">{formatPlayCount(activeSummary.totalPlay)}</p>
                  </div>
                </div>
              </div>
            </>
          ) : (
            <div className="rounded-xl border border-stone-200 bg-stone-50 p-4 text-[13px] text-stone-700">
              点击任意热力单元格查看该时段表现。
            </div>
          )}
        </aside>
