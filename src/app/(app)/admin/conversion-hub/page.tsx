...
import { ConversionHubShell, type HubTabKey } from "./hub-shell";
...
const VALID_TABS: HubTabKey[] = ["scripts", "violations", "weekly", "analytics", "advice"];
...
function normalizeTab(value: string | undefined): HubTabKey {
...
async function loadViolationsTab(params: {
  status: StatusFilter;
  category: string;
  keyword: string;
...
async function loadWeeklyTab(weekStart: string) {
  const supabase = createAdminClient();
...
async function loadAnalyticsTab(sort: SortBy, format: FormatFilter) {
  const supabase = createAdminClient();
  let query = supabase
    .from("violation_cases")
