  ...
  return (
    <div className="relative flex items-center gap-3 rounded-2xl border border-zinc-200 bg-white px-4 py-3">
      ...
      <div className="leading-tight">
        ...
        <p className={cn("mt-0.5 text-[22px] font-semibold tabular-nums", accent.text)}>{tile.count}</p>
        <p className="text-[11px] text-zinc-400">{tile.hint}</p>
      </div>
    </div>
  );
}
...
function ScriptItemRow({ entry, suffix }: { entry: InboxBucketEntry; suffix?: React.ReactNode }) {
  return (
    <Link
      ...
      className="group flex items-start gap-3 rounded-xl border border-transparent px-2.5 py-2 transition-colors hover:border-zinc-200 hover:bg-zinc-50"
    >
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-x-2.5 gap-y-1 text-[11px] text-zinc-500">
          ...
          {entry.risk_level && entry.risk_level !== "low" ? (
            <>
              <span className="text-zinc-300">·</span>
              <span className={cn("rounded-full px-1.5 py-0.5 text-[10px] font-semibold", RISK_TONE[entry.risk_level] ?? "")}>
    ...
    <section className="space-y-4 rounded-2xl border border-zinc-200 bg-white p-5 sm:p-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 border-l-2 border-[#D97757] pl-3">
          <h2 className="text-[16px] font-semibold tracking-tight text-zinc-800">话术资产</h2>
          <span className="text-[11px] text-zinc-500">展示/涨粉/转化由员工填报后自动汇总</span>
        </div>
      </header>
      ...
      <div className="space-y-2">
        <header className="flex items-center justify-between">
          <h3 className="text-[13px] font-semibold text-zinc-800">
            TOP 转化话术
            <span className="ml-2 text-[11px] font-normal text-zinc-500">加权转化率，使用 ≥3 且展示 ≥1k</span>
          </h3>
        </header>
...
function TopScriptRow({ row, rank }: { row: ScriptsTopRow; rank: number }) {
  ...
  return (
    <Link
      ...
      className="group flex items-start gap-2 rounded-xl border border-transparent px-2 py-2 transition-colors hover:border-zinc-200 hover:bg-zinc-50"
    >
...
function ScriptKpi({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="rounded-xl border border-zinc-100 bg-zinc-50/50 px-3 py-3">
      ...
      <p className="mt-1 text-[20px] font-semibold tabular-nums text-zinc-800">{value}</p>
    </div>
  );
}
        ...
        renderItem={(entry) => (
          <ScriptItemRow
            ...
            suffix={
              <>
                <span className="text-zinc-300">·</span>
                <span className="text-[11px] text-zinc-500">展示 {formatNumber(entry.total_views)}</span>
    ...
    <AdminWorkspaceLayout
      ...
      actions={
        <div className="flex flex-wrap gap-2">
          <Link
            href="/violations"
            className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-zinc-200 bg-white px-3 text-[12px] font-medium text-zinc-600 transition-colors hover:border-zinc-300 hover:text-zinc-800"
          >
          ...
          <Link
            ...
            className="inline-flex h-9 items-center gap-1.5 rounded-xl bg-zinc-900 px-3 text-[12px] font-medium text-white transition-colors hover:bg-zinc-800"
          >
