...
export function AiAlertPanel({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-[#ECE7DE] bg-[#F5F3EE]/80">
      <header
        className={cn(
          "flex items-center justify-between gap-3 px-4 py-2.5",
          showPanelBody && "border-b border-[#ECE7DE]",
        )}
      >
        <button
          type="button"
        ...
        >
          <span className="flex shrink-0 items-center gap-1.5">
            <Sparkles className="size-3.5 text-[#78716C]" strokeWidth={1.75} />
            <span className="text-[12px] font-medium tracking-tight text-[#292524]">AI 速览</span>
          </span>
              ...
              {SEVERITY_ORDER.map((sev) => {
                ...
                return (
                  <span
                    key={sev}
                    className={cn(
                      "inline-flex items-center gap-1 rounded-md bg-[#F5F3EE] px-1.5 py-0.5 text-[12px] font-medium",
                      SEVERITY_TEXT[sev],
                    )}
                  >
          ...
          {topAlertSummary && !showPanelBody ? (
            <span className="min-w-0 flex-1 truncate text-[12px] text-[#78716C]">
              · {topAlertSummary}
            </span>
          ) : null}
          {totalAlerts > 0 ? (
            <span className="ml-auto shrink-0 text-[#78716C]">
              {showPanelBody ? (
            ...
            <span className="ml-auto shrink-0 text-[12px] text-[#78716C]">
              今日暂无待办
            </span>
          )}
        </button>
        <div className="flex shrink-0 items-center gap-2">
          {relative ? (
            <span className="text-[12px] text-[#78716C]">{relative}</span>
          ) : null}
          {totalAlerts > 0 ? (
            <button
              type="button"
          ...
          <button
            ...
            className="flex size-6 items-center justify-center rounded-md text-[#78716C] transition hover:bg-[#F5F3EE] hover:text-[#1C1917] disabled:cursor-not-allowed disabled:opacity-60"
            aria-label="刷新告警"
          >
            <RefreshCw
      ...
      {state.loading ? (
        <div className="px-4 py-2">
          <span className="text-[12px] text-[#78716C]">正在加载 AI 速览…</span>
        </div>
      ) : state.error ? (
        <div className="px-4 py-2 flex items-center gap-2">
          ...
          <button
            ...
            className="rounded-md border border-[#E5E0D6] bg-white px-2 py-0.5 text-[12px] font-medium text-[#292524] transition hover:border-[#E5E0D6] hover:text-[#1C1917]"
          >
            重试
          </button>
        </div>
        ...
        <div className="flex md:h-[480px]">
          <aside
            className={cn(
              "min-w-0 md:w-[32%] md:min-w-[260px] md:max-w-[340px] md:flex-shrink-0 md:overflow-y-auto md:border-r md:border-[#E5E0D6]",
                  ...
                  <li key={group.groupKey}>
                    <button
                      ...
                      className={cn(
                        ...
                        active ? "border-l-[#D97757] bg-[#E5E0D6]/95" : "border-l-transparent hover:bg-[#F5F3EE]",
                      )}
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="shrink-0 text-[12px] font-normal tabular-nums text-[#78716C]">
                            {group.count}
                          </span>
                        </div>
          ...
          >
            {!activeGroup ? (
              <div className="flex flex-1 items-center justify-center p-6 text-[12px] text-[#78716C]">
                选择左侧分组查看详情
              </div>
            ) : (
              <>
                <div className="flex items-center gap-2 border-b border-[#ECE7DE] px-3.5 py-2 md:hidden">
                  <button
                    type="button"
                    onClick={() => setMobileView("list")}
                    className="active:translate-y-0 inline-flex items-center gap-1 rounded-lg px-1.5 py-0.5 text-[12px] text-[#78716C] transition-[color,background-color] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)] hover:bg-[#F5F3EE] hover:text-[#1C1917]"
                  >
                ...
                <div className="sticky top-0 z-10 flex items-center gap-2 border-b border-[#ECE7DE] bg-[#FBF9F5]/90 px-3.5 py-1.5 backdrop-blur">
                  <Checkbox
                    aria-label="全选"
                    checked={
                  ...
                  <div className="ml-auto flex gap-1.5">
                    <button
                      ...
                      className="inline-flex h-6 items-center rounded-md border border-[#E5E0D6] px-2.5 text-[12px] text-[#292524] transition hover:border-[#E5E0D6] disabled:cursor-not-allowed disabled:opacity-50"
                    >
                ...
                <ul className="flex-1 divide-y divide-[#ECE7DE] md:overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#E5E0D6] [&::-webkit-scrollbar-track]:bg-transparent">
                    ...
                    return (
                      <li key={alert.id}>
                        {navigate ? (
                          <Link
                            href={navigate.href!}
                            className="active:translate-y-0 group flex h-9 items-center gap-3 px-4 transition-[background-color] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)] hover:bg-[#FBF9F5]/60"
                          >
                            <div
                              onClick={(e) => {
                                e.preventDefault();
                            ...
                            <span className="min-w-[80px] truncate text-[13px] font-medium text-[#1C1917]">
                              {primaryEntity?.name ?? "—"}
                            </span>
                            <span className="flex-1 truncate text-[12px] text-[#78716C]">
                              {alert.detail ?? alert.title}
                            </span>
                            <ArrowRight className="size-3.5 text-[#78716C] opacity-0 transition duration-150 group-hover:opacity-100" />
                          </Link>
                        ) : (
                          <div
                            className="group flex h-9 items-center gap-3 px-4 transition-[background-color] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)] hover:bg-[#FBF9F5]/60"
                          >
                            <div
                              onClick={(e) => {
                                e.stopPropagation();
