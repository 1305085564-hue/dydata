...
export function SystemLogTicker({ logs }: SystemLogTickerProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-zinc-200 px-4 py-2 text-[12px] flex justify-center overflow-hidden h-9">
      <div className="max-w-[80rem] w-full flex items-center justify-between">
        ...
        >
          <span className="font-medium text-zinc-900">{currentLog.profile_name || "系统"}</span>
          <span className="text-zinc-500">{currentLog.action}</span>
          {currentLog.target && <span className="text-zinc-500 truncate max-w-[200px]">({currentLog.target})</span>}
          <span className="text-[12px] text-zinc-500 ml-2 bg-zinc-100 px-1.5 py-0.5 rounded-full tabular-nums">
