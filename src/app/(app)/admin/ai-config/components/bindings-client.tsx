  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-[18px] font-medium text-zinc-900">业务功能绑定</h2>
            ...
            {bundle.featureBindings.length === 0 ? (
              ...
              bundle.featureBindings.map((binding) => (
                <TableRow key={binding.id}>
                  <TableCell className="font-mono text-[12px] text-zinc-700">
                    {binding.feature_key}
                  </TableCell>
                  ...
                  <TableCell>
                    <Badge variant="outline" className="font-normal text-zinc-700 bg-zinc-50">
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="icon" aria-label={`编辑绑定 ${binding.label}`} className="size-8 text-zinc-500 hover:text-zinc-700" onClick={() => setBindingModal({ open: true, data: binding })}>
                      <Pencil className="size-4" />
                    </Button>
                    <Button variant="ghost" size="icon" aria-label={`删除绑定 ${binding.label}`} className="size-8 text-zinc-500 hover:text-[#C9604D]" onClick={() => setDeleteConfirm({ open: true, id: binding.id, title: `删除功能绑定 ${binding.label}` })}>
