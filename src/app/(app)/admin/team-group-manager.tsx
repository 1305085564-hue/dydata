      ...
      <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Select value={selectedTeamId} onValueChange={handleTeamChange}>
            <SelectTrigger className="h-10 w-full bg-zinc-50 border-transparent focus:bg-white focus:border-zinc-500 focus:shadow-sm focus:ring-1 focus:ring-zinc-900/5 transition-[background-color,border-color,box-shadow] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)] sm:w-48">
          ...
          <div className="space-y-2">
            {teamGroups.length > 0 ? (
              teamGroups.map((group) => (
                ...
                >
                  <span>
                    <span className="block font-medium text-zinc-900">{group.name}</span>
              ...
              <div className="rounded-xl border border-dashed border-zinc-300 bg-zinc-50 p-4 text-[13px] text-zinc-500">
                这个团队还没有分组。
              </div>
            )}
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {directMembers.length > 0 ? (
                directMembers.map((member) => (
                  <Badge key={member.id} className="bg-zinc-100 text-zinc-700">
                    {member.name}
                  </Badge>
                ))
              ) : (
                <span className="text-[13px] text-zinc-500">暂无直管组员</span>
              )}
            </div>
          </div>
        </div>
            ...
            <div className="grid gap-2 rounded-2xl border border-zinc-200 bg-white p-4 md:grid-cols-[1fr_180px_auto]">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="例如：第一组"
                  className="rounded-xl bg-zinc-50 border-transparent focus:bg-white focus:border-zinc-500 focus:shadow-sm focus:ring-1 focus:ring-zinc-900/5 transition-[background-color,border-color,box-shadow] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)]"
                />
              </div>
              <div className="space-y-1.5">
                <Label id="new-group-leader-label">组长</Label>
                <Select value={newLeaderId} onValueChange={handleNewLeaderChange}>
                  <SelectTrigger aria-labelledby="new-group-leader-label" className="h-10 w-full bg-zinc-50 border-transparent focus:bg-white focus:border-zinc-500 focus:shadow-sm focus:ring-1 focus:ring-zinc-900/5 transition-[background-color,border-color,box-shadow] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)]">
          ...
          {currentGroup ? (
            <div className="space-y-4">
                ...
                <div className="grid gap-2 rounded-2xl border border-zinc-200 bg-white p-4 md:grid-cols-[1fr_180px_auto]">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={editLeaderId} onValueChange={handleEditLeaderChange}>
                      <SelectTrigger aria-labelledby="edit-group-leader-label" className="h-10 w-full bg-zinc-50 border-transparent focus:bg-white focus:border-zinc-500 focus:shadow-sm focus:ring-1 focus:ring-zinc-900/5 transition-[background-color,border-color,box-shadow] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)]">
                        <SelectValue placeholder="选择组长">
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                <div className="rounded-xl border border-zinc-200 bg-white p-4">
                  <div className="mb-2 font-medium text-zinc-900">组内成员</div>
                  <div className="space-y-2">
                ...
                {access.canEditGroups ? (
                  <div className="rounded-xl border border-zinc-200 bg-white p-4">
                    <div className="mb-2 flex items-center justify-between gap-2">
                      <div className="font-medium text-zinc-900">分配组员</div>
                      <Button
                        type="button"
                        size="sm"
                        variant="outline"
 className="bg-white border-zinc-300 text-zinc-900 hover:bg-zinc-50 active:translate-y-0 "
                    ...
                    <div className="max-h-72 space-y-4 overflow-y-auto pr-1">
                      ...
                      {unassignedMembers.length > 0 && (
                        <div className="space-y-2">
                          <div className="text-[12px] font-normal text-zinc-500">未分配</div>
                          {unassignedMembers.map((member) => (
                            ...
                            >
                              ...
                              <Badge className="bg-zinc-100 text-zinc-700">未分配</Badge>
                            </label>
                          ))}
                        </div>
                      )}
                        ...
                        <div className="space-y-2">
                          {unassignedMembers.length > 0 && (
                            <div className="border-t border-zinc-200 pt-2">
                              <div className="text-[12px] font-normal text-zinc-500">已分配</div>
                            </div>
                          )}
            ...
            <div className="rounded-xl border border-dashed border-zinc-300 bg-zinc-50 p-6 text-[13px] text-zinc-500">
              先创建或选择一个分组。
            </div>
          )}
        </div>
