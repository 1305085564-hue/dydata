...
const ACTION_CONFIG: Record<MarkAction, ActionConfig> = {
  leave: { label: "标记请假", variant: "outline", colorClass: "border-stone-200 text-stone-700 hover:bg-stone-50 hover:text-stone-900" },
  waived: { label: "标记豁免", variant: "outline", colorClass: "border-stone-200 text-stone-700 hover:bg-stone-50 hover:text-stone-900" },
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-lg border px-2 py-1 text-[12px] font-medium ${c.border} ${c.bg} text-stone-700`}>
    ...
    return (
      <Sheet open={open} onOpenChange={handleOpenChange}>
        <SheetContent side="right" className="w-[480px]">
          ...
          <SheetBody>
            <p className="text-[13px] text-stone-500">未选择成员</p>
          </SheetBody>
        </SheetContent>
      </Sheet>
    );
                ...
                {dateAppeal.status === "pending" && (
                  <div className="flex gap-2 pt-1">
                    <Button
                      variant="outline"
                      size="sm"
                      className="flex-1 border-stone-200 text-stone-700 hover:bg-stone-50 hover:text-stone-900 font-medium"
                ...
                {dayRecord ? (
                  <div className="space-y-2.5">
                    ...
                    {dayRecord.markedByName ? (
                      <div className="flex items-center justify-between text-[13px]">
                        <span className="text-stone-500">标记人</span>
                        <span className="text-stone-700 font-medium">{dayRecord.markedByName}</span>
                      </div>
                    ) : null}
                    {dayRecord.markedAt ? (
                      <div className="flex items-center justify-between text-[13px]">
                  ...
                  <p className="text-[13px] text-stone-500">当日无记录</p>
                )}
              </div>
            </section>
              ...
              {activeAction ? (
                ...
                <div className="space-y-3">
                  ...
                  {dayRecord && dayRecord.status !== "published" && dayRecord.status !== "exempted" && dayRecord.status !== "unconfirmed" && (
                    <Button
                      variant="outline"
                      className="w-full border-stone-200 text-stone-700 hover:bg-stone-50 hover:text-stone-900"
