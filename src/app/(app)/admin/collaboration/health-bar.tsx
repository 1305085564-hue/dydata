...
export function HealthBar({ summary }: HealthBarProps) {
  ...
  const healthRate = Math.round(((summary.total - summary.unattributed) / (summary.total || 1)) * 100);
  ...
  return (
    <>
      ...
      <button
        ...
        className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[12px] font-medium transition-all border ${
          isHealthy
            ? "border-emerald-200/80 bg-emerald-50/60 text-emerald-800 hover:bg-emerald-100/70"
            : "border-amber-200/80 bg-amber-50/60 text-amber-900 hover:bg-amber-100/70"
        }`}
      >
        {isHealthy ? (
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-md p-5 rounded-2xl border border-zinc-200 bg-white shadow-xl">
          ...
          <div className="space-y-3 pt-2 text-[13px] text-zinc-700 leading-relaxed">
            <p>
              本月共有 <strong className="font-semibold text-zinc-900">{summary.total}</strong> 条作品。
              其中 <strong className="font-semibold text-amber-700">{summary.unattributed}</strong> 条作品缺乏明确的文案、剪辑或运营归属。
            </p>
            {neverFillCount > 0 && (
              <div className="rounded-xl border border-amber-200/60 bg-amber-50/40 p-3 space-y-1">
                <div className="font-medium text-amber-950 text-[12px]">仅标注自运营的成员：</div>
