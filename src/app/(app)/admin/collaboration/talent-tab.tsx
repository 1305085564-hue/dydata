...
export function TalentTab({ talents, onSelectPerson, onPrefetchPerson }: TalentTabProps) {
  ...
  const sorted = useMemo(() => {
    const list = [...talents];
    list.sort((a, b) => {
      const diff = sortOrder === "desc" ? b[sortField] - a[sortField] : a[sortField] - b[sortField];
  ...
  if (talents.length === 0) {
    return (
      <div className="rounded-2xl border border-zinc-200 bg-white p-8 text-center shadow-2xs">
        ...
        <p className="mt-3 text-[14px] font-medium text-zinc-500">本月暂无达人数据</p>
        <p className="mt-1 text-[12px] text-zinc-400">达人 = 名下有账号的出镜成员</p>
      </div>
    );
  }
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr className="border-b border-zinc-100 bg-zinc-50/60">
              <th className="py-2.5 pl-4 pr-2 text-left font-medium text-zinc-500 w-[140px]">达人</th>
              <th
              ...
              <th className="py-2.5 px-2 text-left font-medium text-zinc-500">名下账号</th>
            </tr>
          </thead>
          <tbody>
            {sorted.map((row) => (
              ...
              >
                ...
                <td className="py-2.5 px-2 text-right tabular-nums text-zinc-700">{row.accountCount}</td>
                <td className="py-2.5 px-2 text-right tabular-nums text-zinc-700">{row.reportCount}</td>
                <td className="py-2.5 px-2 text-right tabular-nums text-zinc-700">{formatBigNumber(row.totalPlay)}</td>
                <td className="py-2.5 px-2 text-right tabular-nums text-zinc-700">
                  {row.hitCount > 0 ? (
                    <span className="text-red-600 font-medium">{row.hitCount}</span>
                  ) : (
