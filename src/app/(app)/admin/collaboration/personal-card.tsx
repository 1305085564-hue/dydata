      ...
      >
        ...
        <DialogHeader className="p-5 pb-3.5 border-b border-zinc-100 flex flex-row items-center justify-between shrink-0 bg-zinc-50/40">
          {loading ? (
            ...
            <div>
              <DialogTitle className="text-[16px] font-semibold text-[#DC2626]">
                加载失败
              </DialogTitle>
                  ...
                  {data.operatorSummary ? (
                    <div className="grid grid-cols-2 gap-2 text-center pt-1">
                      ...
                      <div className="rounded-lg border border-zinc-200 bg-white p-2">
                        ...
                        <div className="text-[13px] font-semibold text-zinc-600 tabular-nums mt-0.5">
                    ...
                    <table className="w-full text-[12px]">
                      <thead className="sticky top-0 bg-zinc-50/95 backdrop-blur-xs border-b border-zinc-100 text-zinc-500 text-left z-10">
                        <tr>
                          ...
                          <th className="py-2 px-3 font-medium text-right pr-3">
                            状态
                          </th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-zinc-100">
                        {data.records.map((rec) => (
                          <tr
                            key={rec.reportId}
                            className="hover:bg-zinc-50/60 transition-colors h-9"
                          >
                            <td className="py-1.5 px-3 text-zinc-500 tabular-nums whitespace-nowrap">
                              {rec.reportDate}
                            </td>
                            <td className="py-1.5 px-3">
                              ...
                              <div
                                className="text-[11px] text-zinc-400 max-w-[340px] truncate"
                                title={rec.title}
                              >
                                {rec.title}
                              </div>
                                ...
                                {rec.roles.map((r) => {
                                  if (r === "writer") {
                                    return (
                                      <span
                                        key={r}
                                        className="rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] font-medium text-zinc-600 border border-zinc-200"
                                      >
                                        文案
                                      </span>
                                    );
