      ...
      <div className="flex flex-col items-center gap-2 pt-3">
        <div className="flex flex-wrap items-center justify-center gap-1">
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Button key={p} size="sm" variant={p === page ? "default" : "outline"} onClick={() => onPageChange(p)} className={`h-8 w-8 p-0 text-[12px] rounded-xl ${p === page ? "bg-white border-[#D97757]/40 text-[#D97757] hover:bg-white hover:border-[#D97757]/60" : "bg-white border-zinc-200 text-zinc-700 hover:bg-zinc-50"}`}>{p}</Button>
          ))}
      ...
      <div className="grid grid-cols-2 gap-5 sm:grid-cols-4">
        <Card className="bg-white border-zinc-200">
          <CardContent className="pt-6 pb-5">
            <p className="text-[12px] uppercase tracking-[0.25em] font-medium text-zinc-500">{summary.totalLabel}</p>
            <p className="text-[18px] font-medium text-zinc-900 tracking-tight mt-2 tabular-nums">
        ...
        <Card className="bg-white border-zinc-200">
          <CardContent className="pt-6 pb-5">
            <p className="text-[12px] uppercase tracking-[0.25em] font-medium text-zinc-500">提交率</p>
            <p className="text-[18px] font-medium text-zinc-900 tracking-tight mt-2 tabular-nums">
          ...
          {viewMode === "profile" ? (
            <>
                  ...
                  <TableBody>
                      ...
                      <TableRow key={row.id} className="bg-zinc-50/50 hover:bg-zinc-50">
                        ...
                        <TableCell>
                          {row.ownAccounts.length ? (
                            ...
                            <span className="text-[12px] text-zinc-500">暂无账号</span>
                          )}
                        </TableCell>
                    ...
                    {submittedProfileRowsList.length > 0 && (
                      <>
                        ...
                        >
                          <TableCell colSpan={7} className="py-2 text-[12px] text-zinc-500 select-none">
                        ...
                        {profileSubmittedExpanded && submittedProfileRowsList.map((row) => (
                          <TableRow key={row.id} className="hover:bg-zinc-50">
                            ...
                            <TableCell>{renderExemptionHint(row.exemption.label, row.exemption.reason)}</TableCell>
                            <TableCell className="text-right">
                              <Button variant="ghost" size="sm" onClick={() => openDialog(row)} className="text-[12px] text-zinc-500">设置豁免</Button>
                            </TableCell>
                          </TableRow>
                        ))}
                      </>
                    ...
                    {profileRows.exemptRows.length > 0 && (
                      <>
                        <TableRow>
                          <TableCell colSpan={7} className="border-0 pt-4 pb-1 text-[12px] text-zinc-500">
                            豁免人员（不计入提交统计）
                          </TableCell>
                        </TableRow>
                        {profileRows.exemptRows.map((row) => (
                          <TableRow key={row.id} className="bg-zinc-50 text-zinc-500 hover:bg-zinc-50">
                            <TableCell>{row.name}</TableCell>
                            <TableCell>
                              <Badge variant="outline" className="text-[12px]">
                                {renderRole(row.role)}
              ...
              <div className="space-y-3 sm:hidden">
                    ...
                    <div className="p-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="space-y-1">
                          <p className="text-[13px] font-medium text-zinc-900">{row.name}</p>
                          <p className="text-[12px] text-zinc-500">{renderRole(row.role)} · {row.accountCount} 个账号</p>
                        </div>
                ...
                {submittedProfileRowsList.length > 0 && (
                  <>
                    <button
                      className="active:translate-y-0 w-full pt-2 text-left text-[12px] text-zinc-500 hover:text-zinc-900"
                      onClick={() => setProfileSubmittedExpanded((v) => !v)}
                    >
                      已提交 {submittedProfileRowsList.length} 人 {profileSubmittedExpanded ? "▲" : "▼"}
                    </button>
                        ...
                        <div className="p-3">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="flex flex-col items-end gap-2">
                              ...
                              <Button variant="ghost" size="sm" onClick={() => openDialog(row)} className="h-7 px-2 text-[12px] text-zinc-500">豁免</Button>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                {profileRows.exemptRows.length > 0 && (
                  <>
                    <p className="pt-2 text-[12px] text-zinc-500">豁免人员</p>
                    {profileRows.exemptRows.map((row) => (
                      <div key={row.id} className="space-y-2 rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-zinc-500">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="text-[13px] font-medium">{row.name}</p>
                            <p className="text-[12px] text-zinc-500">{renderRole(row.role)} · {row.accountCount} 个账号</p>
            ...
            <>
                  ...
                  <TableBody>
                      ...
                      <TableRow key={row.id} className="bg-zinc-50/50 hover:bg-zinc-50">
                        ...
                        <TableCell>
                          <div className="space-y-1">
                            ...
                            <p className="text-[12px] text-zinc-500">{renderRole(row.profile.role)}</p>
                          </div>
                        </TableCell>
                    ...
                    {submittedAccountRowsList.length > 0 && (
                      <>
                        ...
                        >
                          <TableCell colSpan={6} className="py-2 text-[12px] text-zinc-500 select-none">
                        ...
                        {accountSubmittedExpanded && submittedAccountRowsList.map((row) => (
                          <TableRow key={row.id} className="hover:bg-zinc-50">
                            <TableCell>{row.name}</TableCell>
                            <TableCell>
                              <div className="space-y-1"><p>{row.profile_name}</p><p className="text-[12px] text-zinc-500">{renderRole(row.profile.role)}</p></div>
                            </TableCell>
                            <TableCell>{renderAccountMeta(row)}</TableCell>
                            <TableCell><Badge variant="success" className="text-[12px]">已提交</Badge></TableCell>
                            <TableCell>{renderExemptionHint(row.exemption.label, row.exemption.reason)}</TableCell>
                            <TableCell className="text-right">
                              <Button variant="ghost" size="sm" onClick={() => openDialog(row.profile)} className="text-[12px] text-zinc-500">设置豁免</Button>
                            </TableCell>
                          </TableRow>
                        ))}
                      </>
                    ...
                    {accountRows.exemptRows.length > 0 && (
                      <>
                        <TableRow>
                          <TableCell colSpan={6} className="border-0 pt-4 pb-1 text-[12px] text-zinc-500">
                            豁免账号（继承所属人员豁免状态）
                          </TableCell>
                        </TableRow>
              ...
              <div className="space-y-3 sm:hidden">
                  ...
                  >
                    <div className="p-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="space-y-1">
                          <p className="text-[13px] font-medium text-zinc-900">{row.name}</p>
                          <p className="text-[12px] text-zinc-500">{row.profile_name} · {renderRole(row.profile.role)}</p>
                        </div>
                        <div className="flex flex-col items-end gap-2">
                        ...
                        <div className="p-3">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="flex flex-col items-end gap-2">
                              ...
                              <Button variant="ghost" size="sm" onClick={() => openDialog(row.profile)} className="h-7 px-2 text-[12px] text-zinc-500">豁免</Button>
                            </div>
                          </div>
                          <div className="mt-3 space-y-2 text-[12px] text-zinc-500">
                            {renderAccountMeta(row)}
                ...
                {accountRows.exemptRows.length > 0 && (
                  <>
                    <p className="pt-2 text-[12px] text-zinc-500">豁免账号</p>
                    {accountRows.exemptRows.map((row) => (
                      <div key={row.id} className="space-y-2 rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-zinc-500">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="text-[13px] font-medium">{row.name}</p>
                            <p className="text-[12px] text-zinc-500">{row.profile_name}</p>
                          </div>
                          <Button
                            variant="ghost"
                            size="sm"
