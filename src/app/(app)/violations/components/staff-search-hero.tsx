    ...
    <section className={`rounded-2xl border border-zinc-200 bg-white p-5 sm:p-6 transition-opacity ${isPending ? "opacity-70" : ""}`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="leading-tight">
          ...
          <p className="text-[13px] text-zinc-500">
            团队已积累 <span className="tabular-nums text-zinc-700">{totalCases}</span> 条案例
          </p>
        </div>
        <Link
          href="/violations/submit"
          className="inline-flex h-10 items-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-4 text-[13px] font-medium text-zinc-700 transition-colors hover:bg-zinc-100 active:translate-y-0"
        >
      ...
      <form onSubmit={handleSubmit} className="relative mt-4">
        ...
        <input
          ...
          disabled={isPending}
          className="h-12 w-full rounded-lg border border-zinc-200 bg-zinc-50 pl-11 pr-28 text-[13px] text-zinc-700 placeholder:text-zinc-500 transition-colors focus:border-zinc-500 focus:bg-white focus:shadow-sm focus:outline-none focus:ring-1 focus:ring-zinc-900/5 disabled:opacity-60"
        />
        <button
          type="submit"
          disabled={isPending}
          className="absolute right-2 top-1/2 inline-flex h-9 -translate-y-1/2 items-center gap-1 rounded-lg bg-[#D97757] px-4 text-[12px] font-medium text-white transition-colors hover:bg-[#C96442] active:translate-y-0 disabled:cursor-not-allowed disabled:border disabled:border-zinc-200 disabled:bg-zinc-100 disabled:text-zinc-500/40 disabled:hover:bg-zinc-100"
        >
          {isPending ? (
            <>
