    ...
    <div className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {data.screenshots.map((s) => (
              <span
                key={s.path}
                className="rounded-lg border border-zinc-200 bg-white px-2.5 py-1 text-[12px] text-zinc-600"
              >
                {s.name}
              </span>
            ))}
      ...
      <div className="grid grid-cols-2 gap-3 text-[13px]">
        <div>
          ...
          <p className="mt-1 font-medium text-zinc-800">{accountLabel}</p>
        </div>
        <div>
          <span className="text-[10px] font-medium uppercase tracking-[0.25em] text-zinc-400">
            提交类型
        ...
        <div className="space-y-3 rounded-lg border border-zinc-200 bg-white p-4">
          <div className="grid grid-cols-2 gap-3 text-[13px]">
            <div>
              ...
              <p className="mt-1 font-medium text-zinc-800">{data.eventType}</p>
            </div>
            <div>
              <span className="text-[10px] font-medium uppercase tracking-[0.25em] text-zinc-400">
                发生时间
              </span>
              <p className="mt-1 font-medium text-zinc-800">{data.occurredAt || "—"}</p>
            </div>
            <div>
              <span className="text-[10px] font-medium uppercase tracking-[0.25em] text-zinc-400">
                申诉状态
              </span>
              <p className="mt-1 font-medium text-zinc-800">{data.appealStatus}</p>
            </div>
          </div>
          <div>
            <span className="text-[10px] font-medium uppercase tracking-[0.25em] text-zinc-400">
        ...
        <div className="space-y-3 rounded-lg border border-zinc-200 bg-white p-4">
          <div className="grid grid-cols-2 gap-3 text-[13px]">
            <div>
              ...
              <p className="mt-1 font-medium tabular-nums text-zinc-800">
          ...
          <div>
            ...
            <div className="mt-1 flex flex-wrap gap-1.5">
              {data.platforms.map((p) => (
                <span
                  key={p}
                  className="rounded-lg border border-zinc-200 px-2 py-0.5 text-[12px] text-zinc-600"
                >
                  {p}
                </span>
              ))}
