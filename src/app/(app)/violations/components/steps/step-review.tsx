  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <blockquote className="rounded-lg border-l-2 border-l-[#D97757]/40 bg-zinc-50/60 p-4 text-[13px] leading-[1.7] text-zinc-700">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {data.screenshots.map((s) => (
              <span
                key={s.path}
                className="rounded-lg border border-zinc-200 bg-white px-2.5 py-1 text-[12px] text-zinc-700"
              >
                {s.name}
              </span>
            ))}
      ...
      <div className="grid grid-cols-2 gap-3 text-[13px]">
        <div>
          ...
          <p className="mt-1 font-medium text-zinc-700">{accountLabel}</p>
        </div>
        <div>
          <span className="text-[12px] font-normal tracking-[0.12em] text-zinc-500">
            提交类型
        ...
        <div className="space-y-3 rounded-lg border border-zinc-200 bg-white p-4">
          <div className="grid grid-cols-2 gap-3 text-[13px]">
            <div>
              ...
              <p className="mt-1 text-[13px] font-normal text-zinc-700">{data.eventType}</p>
            </div>
            <div>
              <span className="text-[12px] font-normal tracking-[0.12em] text-zinc-500">
                发生时间
              </span>
              <p className="mt-1 text-[13px] font-normal tabular-nums text-zinc-700">{data.occurredAt || "—"}</p>
            </div>
            <div>
              <span className="text-[12px] font-normal tracking-[0.12em] text-zinc-500">
                申诉状态
              </span>
              <p className="mt-1 text-[13px] font-normal text-zinc-700">{data.appealStatus}</p>
            </div>
          </div>
          <div>
            <span className="text-[12px] font-normal tracking-[0.12em] text-zinc-500">
            ...
            <p className="mt-1 whitespace-pre-wrap text-[13px] leading-[1.7] text-zinc-700">
        ...
        <div className="space-y-3 rounded-lg border border-zinc-200 bg-white p-4">
          <div className="grid grid-cols-2 gap-3 text-[13px]">
            <div>
              ...
              <p className="mt-1 text-[13px] font-normal tabular-nums text-zinc-700">
          ...
          <div>
            ...
            <div className="mt-1 flex flex-wrap gap-1.5">
              {data.platforms.map((p) => (
                <span
                  key={p}
                  className="rounded-lg border border-zinc-200 px-2 py-0.5 text-[12px] text-zinc-500"
                >
                  {p}
                </span>
              ))}
      ...
      {/* Warning */}
      <div className="flex items-start gap-2 rounded-lg border border-[#D99E55]/30 bg-zinc-50/60 p-3">
