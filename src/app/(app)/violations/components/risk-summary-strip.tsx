        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="rounded-xl border border-zinc-200 bg-white p-4">
            ...
            <div className="mt-2 flex items-baseline gap-4">
              <div>
                <span className="text-[18px] font-medium tabular-nums text-[#C9604D]">
                  {data.weeklyStats.newViolations}
                </span>
                <span className="ml-1 text-[12px] text-zinc-500">条违规</span>
              </div>
              <div>
                <span className="text-[18px] font-medium tabular-nums text-zinc-700">
          ...
          {data.dangerousTop3.length > 0 && (
            <div className="rounded-xl border border-zinc-200 border-l-[2px] border-l-[#C9604D] bg-white p-4">
              <p className="flex items-center gap-1.5 text-[12px] font-medium tracking-[0.12em] text-[#C9604D]">
                ...
                {data.dangerousTop3.map((c) => (
                  <li key={c.id}>
                    <Link
                      ...
                      className="flex items-center justify-between gap-2 rounded-lg px-2 py-1 text-[12px] transition-colors hover:bg-zinc-100"
                    >
                      <span className="truncate text-zinc-700">{c.script_text}</span>
                      <span className="shrink-0 font-medium tabular-nums text-[#C9604D]">
                        {c.pass_rate ?? 0}%
                      </span>
                    </Link>
                  </li>
          ...
          {data.safeTop3.length > 0 && (
            <div className="rounded-xl border border-zinc-200 border-l-[2px] border-l-[#6FAA7D] bg-white p-4">
              <p className="flex items-center gap-1.5 text-[12px] font-medium tracking-[0.12em] text-[#6FAA7D]">
              ...
              <ul className="mt-2 space-y-1.5">
                {data.safeTop3.map((c) => (
                  <li key={c.id}>
                    ...
                    >
                      <span className="truncate text-zinc-700">{c.script_text}</span>
                      <span className="shrink-0 font-medium tabular-nums text-[#6FAA7D]">
                        {c.pass_rate}%
                      </span>
                    </Link>
                  </li>
