...
test("累积期只给进度和招式：进度卡、教练卡、追赶条、锁定雷达，不出诊断", () => {
  ...
  assert.match(html, /写法对照 · 团队验证过 vs 你最近一篇/);
  assert.match(html, /同事的写法 · 真实对标同事/);
  assert.match(html, /你的写法 · 最近一篇（7月15日）/);
  assert.match(html, /我的开头先讲了两句背景/);
...
test("观察期诊断卡回归并标注样本口径，比例指标点亮，同伴为双人对比", () => {
  ...
  assert.doesNotMatch(html, /排行榜 · 全局排行/);
  assert.doesNotMatch(html, /最新视频文案拆解/);
});
