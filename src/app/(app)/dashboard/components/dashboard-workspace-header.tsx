    ...
    <div className="mx-auto mb-6 max-w-5xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="min-w-0 space-y-1">
          <div className="flex items-center gap-1.5 text-[12px] font-medium uppercase tracking-[0.25em] text-[#78716C]">
            <Activity size={13} className="text-[#78716C]" /> 数据台
          </div>
          ...
          <div className="relative inline-flex items-center" ref={calendarPopoverRef}>
            <button
              ...
              className="group inline-flex items-center gap-2 rounded-xl text-left outline-none select-none cursor-pointer transition-colors active:scale-[0.985] active:duration-75"
            ...
            >
              <h1 className="text-2xl font-semibold tracking-tight text-[#1C1917] group-hover:text-[#D97757] transition-colors flex items-center gap-2">
                {activeBizDate === today ? (
                  <>
                    <span>今日提交</span>
                    <span className="text-base sm:text-lg font-normal text-[#78716C] group-hover:text-[#292524] tabular-nums transition-colors">
                      · {activeBizDate}
                    </span>
                  </>
                ) : (
            ...
            {isCalendarOpen && (
              <div className="absolute left-0 top-full mt-2.5 z-50 animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-150">
                <div className="w-[330px] rounded-2xl border border-[#E5E0D6] bg-white/98 p-2.5 shadow-claude-float backdrop-blur-2xl ring-1 ring-black/5">
                  <SubmissionCalendar
                    today={today}
          ...
          {utilityActions.map((action) => {
            ...
            return (
              <button
                ...
                className="group inline-flex items-center gap-1.5 rounded-xl border border-[#E5E0D6]/80 bg-[#FBF9F5]/80 px-3 py-1.5 text-[13px] font-medium text-[#292524] transition-colors duration-100 hover:border-[#E5E0D6] hover:bg-white hover:text-[#1C1917] hover:shadow-2xs cursor-pointer"
              >
                <Icon
                  size={14}
