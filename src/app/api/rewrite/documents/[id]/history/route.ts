...
import { getDocumentHistoryState, moveDocumentHistoryPointer } from "@/lib/rewrite/documents";
import {
  errorResponse,
  jsonResponse,
  parseJsonBody,
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const state = await getDocumentHistoryState(createServiceClient(), conversationId);
    if (!state) return errorResponse("Document 不存在", 404);
    return jsonResponse(state);
  } catch (error) {
    return errorResponse(error instanceof Error ? error.message : "获取历史状态失败");
  }
}
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const bodyResult = await parseJsonBody<{ action?: unknown }>(request);
  ...
  try {
    const state = await moveDocumentHistoryPointer(createServiceClient(), {
      conversationId,
